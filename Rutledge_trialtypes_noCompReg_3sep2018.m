function [ons,dur,tr_names,values,missing_onsets] = Rutledge_trialtypes_noCompReg_3sep(fname,startISI)
...
i = 7; tr_names{i} = 'Min EV active';            idx = ~isnan(MinEV) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} =MinEV(idx);
i = 8; tr_names{i} = 'Min EV passive';           idx = ~isnan(MinEV) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 9; tr_names{i} = 'Min EV non-social';        idx = ~isnan(MinEV) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 10; tr_names{i} = 'Ch_diff active';           idx = ~isnan(Ch_diff) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = Ch_diff(idx);
i = 11; tr_names{i} = 'Ch_diff passive';          idx = ~isnan(Ch_diff) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} =Ch_diff(idx);
i = 12; tr_names{i} = 'Ch_diff non-social';       idx = ~isnan(Ch_diff) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = Ch_diff(idx);
% for prediction errors, onsets are the time at which the participant was presented with the outcome
i = 13;  tr_names{i} = 'PE active';              idx = ~isnan(PE) & cond==1;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 14;  tr_names{i} = 'PE passive';             idx = ~isnan(PE) & cond==2;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 15;  tr_names{i} = 'PE n-soc';               idx = ~isnan(PE) & cond==3;         ons{i} = onsOut(idx);   values{i} = PE(idx);
% for outcome differences, onsets are the time at which the participant was presented with the outcome
i = 16; tr_names{i} = 'Outcome self>other active';    idx = outcomeDif>0 & cond==1; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 17; tr_names{i} = 'Outcome self>other passive';   idx = outcomeDif>0 & cond==2; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 18; tr_names{i} = 'Outcome self<other active';    idx = outcomeDif<0 & cond==1; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 19; tr_names{i} = 'Outcome self<other passive';   idx = outcomeDif<0 & cond==2; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
...
i = 20; tr_names{i} = 'happiness';                idx = ~isnan([results(:).happiness]);     ons{i} = onsHap(idx);                     values{i} = [results(idx).happiness];
