function [ons,dur,tr_names,values,missing_onsets] = Rutledge_trialtypes_noCompReg_1aug_Ch2(fname,startISI)
...
i = 2;  tr_names{i} = 'PE<zero passive';             idx = ~isnan(PE) & PE<0 & cond==2;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 3;  tr_names{i} = 'PE<zero n-soc';              idx = ~isnan(PE) & PE<0 & cond==3;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 4;  tr_names{i} = 'PE>zero active';              idx = ~isnan(PE) & PE>0 & cond==1;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 5;  tr_names{i} = 'PE>zero passive';             idx = ~isnan(PE) & PE>0 & cond==2;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 6;  tr_names{i} = 'PE>zero n-soc';              idx = ~isnan(PE) & PE>0 & cond==3;         ons{i} = onsOut(idx);   values{i} = PE(idx);
% for outcome differences, onsets are the time at which the participant was presented with the outcome
i = 7; tr_names{i} = 'Outcome self>other active';    idx = outcomeDif>0 & cond==1; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 8; tr_names{i} = 'Outcome self>other passive';   idx = outcomeDif>0 & cond==2; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 9; tr_names{i} = 'Outcome self<other active';    idx = outcomeDif<0 & cond==1; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 10; tr_names{i} = 'Outcome self<other passive';   idx = outcomeDif<0 & cond==2; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
...
i = 11; tr_names{i} = 'happiness';                idx = ~isnan([results(:).happiness]);     ons{i} = onsHap(idx);                     values{i} = [results(idx).happiness];
% % % i = 15; tr_names{i} = 'Agl active';               idx = ~isnan(Agl_Abs) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Agl_Abs(idx)./max(Agl_Abs));
% % % i = 16; tr_names{i} = 'Agl passive';              idx = ~isnan(Agl_Abs) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Agl_Abs(idx)./max(Agl_Abs));
% % % i = 17; tr_names{i} = 'Agl non-social';           idx = ~isnan(Agl_Abs) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Agl_Abs(idx)./max(Agl_Abs));
i = 12; tr_names{i} = 'Min EV=zero active';       idx = ~isnan(MinEV) & MinEV==0 & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 13; tr_names{i} = 'Min EV=zero passive';      idx = ~isnan(MinEV) & MinEV==0 & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 14; tr_names{i} = 'Min EV=zero non-social';   idx = ~isnan(MinEV) & MinEV==0 & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 15; tr_names{i} = 'Min EV<0 active';          idx = ~isnan(MinEV) & MinEV<0 & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 16; tr_names{i} = 'Min EV<0 passive';         idx = ~isnan(MinEV) & MinEV<0 & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 17; tr_names{i} = 'Min EV<0 non-social';      idx = ~isnan(MinEV) & MinEV<0 & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 18; tr_names{i} = 'Ch_diff active';           idx = ~isnan(Ch_diff) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Ch_diff(idx).^2);
i = 19; tr_names{i} = 'Ch_diff passive';          idx = ~isnan(Ch_diff) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} =(Ch_diff(idx).^2);
i = 20; tr_names{i} = 'Ch_diff non-social';       idx = ~isnan(Ch_diff) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Ch_diff(idx).^2);
