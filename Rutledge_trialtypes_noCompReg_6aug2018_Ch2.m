function [ons,dur,tr_names,values,missing_onsets] = Rutledge_trialtypes_noCompReg_6aug_Ch2(fname,startISI)
...
i = 1;  tr_names{i} = 'PE zero active';              idx = ~isnan(PE) & PE==0 & cond==1;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 2;  tr_names{i} = 'PE zero passive';             idx = ~isnan(PE) & PE==0 &cond==2;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 3;  tr_names{i} = 'PE zero n-soc';              idx = ~isnan(PE) & PE==0 & cond==3;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 4;  tr_names{i} = 'PE Neg active';              idx = ~isnan(PE) & PE<0 & cond==1;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 5;  tr_names{i} = 'PE Neg passive';             idx = ~isnan(PE) & PE<0 & cond==2;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 6;  tr_names{i} = 'PE Neg n-soc';              idx = ~isnan(PE) & PE<0 & cond==3;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 7;  tr_names{i} = 'PE Pos active';              idx = ~isnan(PE) & PE>0 & cond==1;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 8;  tr_names{i} = 'PE Pos passive';             idx = ~isnan(PE) & PE>0 & cond==2;         ons{i} = onsOut(idx);   values{i} = PE(idx);
i = 9;  tr_names{i} = 'PE Pos n-soc';              idx = ~isnan(PE) & PE>0 & cond==3;         ons{i} = onsOut(idx);   values{i} = PE(idx);
% for outcome differences, onsets are the time at which the participant was presented with the outcome
i = 10; tr_names{i} = 'Outcome self>other active';    idx = outcomeDif>0 & cond==1; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 11; tr_names{i} = 'Outcome self>other passive';   idx = outcomeDif>0 & cond==2; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 12; tr_names{i} = 'Outcome self<other active';    idx = outcomeDif<0 & cond==1; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
i = 13; tr_names{i} = 'Outcome self<other passive';   idx = outcomeDif<0 & cond==2; ons{i} = onsOut(idx);   values{i} = outcomeDif(idx);
...
i = 14; tr_names{i} = 'happiness';                idx = ~isnan([results(:).happiness]);     ons{i} = onsHap(idx);                     values{i} = [results(idx).happiness];
i = 15; tr_names{i} = 'Agl active';               idx = ~isnan(Agl_Abs) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  if max(Agl_Abs)~=0; values{i} = (Agl_Abs(idx)./max(Agl_Abs)); else; values{i} = 1-((Agl_Abs(idx)./min(Agl_Abs))); end
i = 16; tr_names{i} = 'Agl passive';              idx = ~isnan(Agl_Abs) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  if max(Agl_Abs)~=0; values{i} = (Agl_Abs(idx)./max(Agl_Abs)); else; values{i} = 1-((Agl_Abs(idx)./min(Agl_Abs))); end
i = 17; tr_names{i} = 'Agl non-social';           idx = ~isnan(Agl_Abs) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  if max(Agl_Abs)~=0; values{i} = (Agl_Abs(idx)./max(Agl_Abs)); else; values{i} = 1-((Agl_Abs(idx)./min(Agl_Abs))); end
i = 18; tr_names{i} = 'Min EV zero active';       idx = ~isnan(MinEV) & MinEV==0 & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 19; tr_names{i} = 'Min EV zero passive';      idx = ~isnan(MinEV) & MinEV==0 & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 20; tr_names{i} = 'Min EV zero non-social';   idx = ~isnan(MinEV) & MinEV==0 & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 21; tr_names{i} = 'Min EV Neg active';          idx = ~isnan(MinEV) & MinEV<0 & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 22; tr_names{i} = 'Min EV Neg passive';         idx = ~isnan(MinEV) & MinEV<0 & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 23; tr_names{i} = 'Min EV Neg non-social';      idx = ~isnan(MinEV) & MinEV<0 & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = MinEV(idx);
i = 24; tr_names{i} = 'Ch_diff active';           idx = ~isnan(Ch_diff) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Ch_diff(idx).^2);
i = 25; tr_names{i} = 'Ch_diff passive';          idx = ~isnan(Ch_diff) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} =(Ch_diff(idx).^2);
i = 26; tr_names{i} = 'Ch_diff non-social';       idx = ~isnan(Ch_diff) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = (Ch_diff(idx).^2);
i = 27;  tr_names{i} = 'CR active';               idx = ~isnan(CR) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = CR(idx);
i = 28;  tr_names{i} = 'CR passive';              idx = ~isnan(CR) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = CR(idx);
i = 29;  tr_names{i} = 'CR n-soc';                idx = ~isnan(CR) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = CR(idx);
i = 30;  tr_names{i} = 'EV active';               idx = ~isnan(EV) & cond==1;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = EV(idx);
i = 31;  tr_names{i} = 'EV passive';              idx = ~isnan(EV) & cond==2;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = EV(idx);
i = 32;  tr_names{i} = 'EV n-soc';                idx = ~isnan(EV) & cond==3;         ons{i} = onsOpt(idx);   dur{i} = RT(idx);  values{i} = EV(idx);
