function [result] = fit_happy_model_Omar_1(mtx,happyscore,constant)

%assumes that each row of the cell array is a different subject

rptfit = 25; %repeat N times jittering the betas
...
if isfield(mtx,'inx')
    ...
    inx = [0.5 0.5 0.5 0.1 10 0.5 0.5]; %dummies 
end
...
lb = [-100 -100 -100 0.1 0.1 -100 -100];
ub = [100 100 100 0.99 50 100 100];
...
result.paramNames = {'Ch_diff','MinEV','PE','tau','Utl'};
if exist('constant','var') && ~isnan(constant)
...
decayvec = tau.^[0:size(data.Agl_Abs,2)-1]; decayvec = decayvec(:);
